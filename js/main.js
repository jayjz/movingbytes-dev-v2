(() => {
  ...
  function initSmoothScroll() {
    $$('a[href^="#"]').forEach((link) => {
      link.addEventListener("click", (event) => {
        ...
        const y =
          target.getBoundingClientRect().top + window.scrollY - navOffset - 16;
  ...
  function initCursor() {
    ...
    $$(
      "a, button, .work-card, .contact-link, .deep-dive-trigger, .glossary-link",
    ).forEach((el) => {
  ...
  function initSystemsPanel() {
    ...
    function closePanel() {
      ...
      const onTransitionEnd = (event) => {
        if (
          event.target !== panel &&
          event.target !== panel.querySelector(".systems-panel__content")
        )
          return;
  ...
  document.addEventListener("click", (e) => {
    if (e.target.classList.contains("code-copy-btn")) {
      const container = e.target.closest(
        ".code-reveal__content, .code-block__body",
      );
