...
const routes = ['ledger.html', 'ledger-revised.html'];
const output = path.resolve(process.env.PROTOTYPE_OUTPUT || 'prototypes/portfolio-v3/screenshots/ledger-revised/final');
let server;
    ...
    for (const route of routes) {
        for (const width of [320, 390, 768, 1440]) {
            ...
            if (width === 390 || width === 1440) {
                await page.screenshot({ path: path.join(output, `${route.replace('.html', '')}-${width}-viewport.png`) });
            }
            ...
            const geometry = await page.evaluate(() => {
                ...
                return { heroAction: bounds('.hero .action'), projectTitle: bounds('h2') };
            });
            ...
            results.push({ route, width, axeViolations: axe.violations.length, geometry, result: 'pass' });
            await context.close();
        }
        ...
        if (route === 'ledger-revised.html') assert.equal(await page.locator('.context-figure').isVisible(), true);
